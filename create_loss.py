...
def select_loss(loss,metric,e_size,num_cls,lr,wd,momentum,to_optim):
	...
	if loss=='triplet':
		...
		criterion = MarginLoss(metric=metric,sampling_method='dist',e_size=e_size,n_classes=num_cls)
...
class Sampler():
	...
	def dist_sampler(self, batch, labels, lower_cutoff=0.35, upper_cutoff=1.25):
	...
	def lifted_sampler(self, batch, labels, lower_cutoff=0.25, upper_cutoff=1.5):
...
class Metric():
	...
	def snr_(self,anchor,b,L):
		...
		tmp =anchor-b
		upper = self.var(tmp)
		lower = self.var(anchor)
		d  = upper/lower
		return d
	
	def re_(self,anchor,b,L):
		'''
	...
	def m_(self,anchor,b,L):
		...
		left = (anchor -b).view(1,L.size(1))  
		
		
		right = torch.matmul(M,left.t())
		d = torch.matmul(left,right)
		return d
	
	def rm_(self,anchor,b,L):
		upper = self.m_(anchor,b,L)
		M = torch.matmul(L.t(),L)
		tmp = anchor.view(-1,1)
		right = torch.matmul(M,tmp) # anchor:1*m , an.t(): m*1
		lower = torch.matmul(anchor.t(),right)
		d = upper / lower
		return d
...
class LiftedLoss(torch.nn.Module):
	...
	def __init__(self, e_size ,margin=1, sampling_method='lifted',metric = 'E'):
		...
		if metric == 'maha' or metric =='rM':
			tmp = 1/e_size
			self.L = torch.nn.Parameter(torch.ones((1,e_size))/e_size) # initialization
		else:
			self.L = None
	...
	def forward(self, batch, labels):
		...
		for Npair in sampled_lifted:  # Npair = 
			...
			d_ap = self.dist.give(anchor,pos,self.L) # cal the dist bet anchor and pos
			
			for neg_idx in Npair[3:]: 
				 
				neg = batch[neg_idx,:]
				d_an += torch.exp(self.margin-self.dist.give(anchor,neg,self.L)) # exp(margin - d_an)
				d_pn += torch.exp(self.margin-self.dist.give(pos,neg,self.L))
			d = torch.nn.functional.relu(d_ap+torch.log(d_an+d_pn))
			d_list.append(d)
			
		loss =torch.stack(d_list)
		
		return torch.mean(loss)*0.5  # 1/ (2*P) Sum(...) P: num_of_pair = batch_size   ;mean()= 1/P * Sum[]
...
class MarginLoss(torch.nn.Module):
	...
	def forward(self, batch, labels):
		...
		for triplet in sampled_triplets:
			...
			pos_dist = self.dist.give(train_triplet['Anchor'],train_triplet['Positive'],self.L)   

			neg_dist = self.dist.give(train_triplet['Anchor'],train_triplet['Negative'],self.L)
...
class TripletLoss(torch.nn.Module):
	...
	def __init__(self, e_size,metric = 'E', margin=1, sampling_method='dist'):
	...
	def triplet_distance(self, anchor, positive, negative):
		...
		"""
		d_ap = self.dist.give(anchor,positive,self.L)
		d_an = self.dist.give(anchor,negative,self.L)
