...
class Metric():
	...
	def m_(self,anchor,b,cov):
		...
		right = torch.mm(cov,left.t())
		
		d = torch.mm(left,right)
		return d.pow(1/2)
	
	def rm_(self,anchor,b,cov):
		upper = self.m_(anchor,b,cov).pow(2)
		
		right = torch.mm(cov,anchor.view(-1,1)) # anchor:1*m , an.t(): m*1
		lower = torch.mm(anchor.view(1,-1),right)
...
class LiftedLoss(torch.nn.Module):
	...
	def forward(self, batch, labels):
		...
		if self.metric_method == 'maha' or self.metric_method == 'rM':
			cov_m = torch.zeros((self.e_size,self.e_size)).cuda()
			#if torch.cuda.is_available():
			#	cov.cuda()
			mean = ( torch.sum(batch,0) /batch.size(0) ).view(-1,1)
			cov_miu = torch.mm(mean,mean.t())
			# calculate the cov_matrix need to be averaged:  # **
			count = torch.tensor(0).cuda()  #**
			for i in range(batch.size(0)):
				for j in range(batch.size(0)):
					
					left = batch[i].view(-1,1)
					right=batch[j].view(1,-1)
					
					cov_m += torch.mm(left,right)
					count +=1
					
			cov = cov_m/count - cov_miu
...
class MarginLoss(torch.nn.Module):
	...
	def __init__(self, e_size,n_classes,lambda_,metric ,margin=0.2, nu=0, beta=1.2, beta_constant=False, sampling_method='dist'):
	...
	def forward(self, batch, labels):
		...
		if self.metric_method == 'maha' or self.metric_method == 'rM':
			...
			cov = None
		if torch.cuda.is_available():  # just because cov=None
			cov.cuda()
