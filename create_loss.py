...
def select_loss(loss,metric,lr,wd,momentum,e_size,num_cls,to_optim):
	...
	if loss=='triplet':
		...
		criterion = TripletLoss(metric=metric,sampling_method='rand',e_size = e_size) #***
...
class Sampler():
	...
	def rand_sampler(self, batch, labels):
		...
		"""
		if isinstance(labels,torch.Tensor) and torch.cuda.is_available(): labels = labels.view(len(labels)).cpu().detach().numpy()
		if isinstance(labels, torch.Tensor): labels = labels.view(len(labels)).detach().numpy()
	...
	def dist_sampler(self, batch, labels, lower_cutoff=0.4, upper_cutoff=1.25):
